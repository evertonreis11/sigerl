 ...
 * @param totalizator: 'true' // sum of values in each column, by defaul true;
	...
	doGraph = function(el){
		...
		colPosition = 0; // for printing colors on simple bar graph
 		...
 		for(var val in data){
 			...
 			lbl = data[val][1];
			valueDetails = data[val][2];
			
			if(valueDetails instanceof Array) 
				isDetailApply = true;
			else
				isDetailApply = false;
...
*/ 			
			...
			if(totalizator == false)
				out += "<div class='graphValue"+el.id+"' id='graphValue"+unique+"'></div>";
 			else
 			...
 			if(valueData instanceof Array){
				...
				if(arr.type=="multi"){
 				...
 				for (i in valueData){
					...
					if(isDetailApply == true)
						o = "<div class='subBars"+el.id+"' onmouseover=javascript:showModalDialog("+valueData[i]+","+valueDetails[i][0]+","+valueDetails[i][1]+"); onmouseleave=javascript:hideModalDialog(); style='height:"+heig+"px; background-color: "+arr.colors[i]+"; left:"+wid*i+"px; color:"+arr.showValuesColor+"; font-size:"+fs+"px' >"+sv+"</div>";
					else
