...
<TR align="right">
	<td>
		...
		<c:if test="${empty TabelaFiltroTag.showSubmit || TabelaFiltroTag.showSubmit eq true}">
			...
			|&nbsp;&nbsp;
			<n:link url="#" onclick="javascript:${tag.dynamicAttributesMap['beforeclear']}$n.clearForm(\"form\");${tag.dynamicAttributesMap['afterclear']}submitForm();" id="btn_limpar">Limpar filtro</n:link>
			<c:if test="${TEMPLATE_showExportLink}">
				|&nbsp;&nbsp;
				<n:submit action="exportar" validate="true" type="link" class="btn_excel">Exportar CSV</n:submit>
			</c:if>
		</c:if>
		<c:if test="${pos_acao_ladoDireito}">
			${acoes}
